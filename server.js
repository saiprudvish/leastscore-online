...
function performMove(room, p, source, leaving, automatic = false) {
  ...
  if (room.round === 1) {
    ...
    if (room.roundReady) room.declarationUnlocked = true;
  }
...
function roomView(room, username) {
  ...
  return {
    ...
    canDeclare: !!room.declarationUnlocked && room.status === "playing" && room.turn === room.players.findIndex(p => p.username === username),
...
io.on("connection", socket => {
  ...
  socket.on("declare", ({ code } = {}) => {
    ...
    const p = findPlayer(room, username);
    if (!room.roundReady) return socket.emit("errorMsg", "Complete one move for every player before declaring.");
  ...
  socket.on("leaveRoom", ({ code } = {}) => {
    ...
    if (leaving.socketId) socket.leave(code);
    room.players.splice(idx, 1);
    if (room.players.length === 0 || !room.players.some(p => !p.bot)) {
      clearTurnTimer(room);
      rooms.delete(code);
