...
function validLeaveGroup(cards) {
  ...
  // Same-rank groups: 2, 3 or 4 of the same rank.
  // (The server supports 4 suits if a standard 52-card deck is enabled.)
  if (cards.length >= 2 && cards.length <= 4 && cards.every(c => c.rank === cards[0].rank)) {
    return true;
  }

  // Sequences are based on RANK ORDER, not suit.  The game allows an
  // ODD number of consecutive ranks: 3, 5, 7, ...
  // Examples: 2-3-4, 4-5-6, 8-9-10, 10-J-Q, A-2-3-4-5, 10-J-Q-K-A.
  // Suits may be mixed (e.g. 2♠ 3♦ 4♣).
  if (cards.length >= 3 && cards.length % 2 === 1) {
    const vals = cards.map(c => rankValue(c.rank));
    if (new Set(vals).size !== vals.length) return false;
    vals.sort((a, b) => a - b);

    // High ace is the only allowed wrap: 10-J-Q-K-A.
    // Check this before low-ace because its sorted values also start at 1.
    if (vals.includes(13) && vals.includes(1)) {
      return vals.length === 5 && vals.join(',') === '1,10,11,12,13';
    }

    // Low-ace sequences: A-2-3-... only.
    if (vals[0] === 1) {
...
function findLeaveGroups(hand) {
  ...
  Object.values(byRank).forEach(g => {
    for (let n = 2; n <= Math.min(4, g.length); n++) groups.push(g.slice(0, n));
  });
...
function roomView(room, username) {
  ...
  return {
    ...
    canDeclare: room.roundMovedBy.has(username) && room.status === "playing" && room.turn === room.players.findIndex(p => p.username === username),
