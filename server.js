...
function validLeaveGroup(cards) {
  ...
  // Same-rank groups: pairs or triples/four-of-a-kind. The 3-suit deck
  // naturally means a triple is the largest same-rank group.
  if (cards.length >= 2 && cards.length <= 4 && cards.every(c => c.rank === cards[0].rank)) return true;

  // Same-suit consecutive sequence. Any length of 3+ is valid:
  // 2-3-4, 8-9-10, 10-J-Q, 8-9-10-J, etc.
  if (cards.length >= 3) {
...
io.on("connection", socket => {
  ...
  socket.on("move", ({ code, source, leaveIds = [] } = {}) => {
    ...
    if (!validLeaveGroup(leaving)) return socket.emit("errorMsg", "Invalid group. Use 1 card, 2–4 same-rank cards, or 3+ consecutive cards of one suit.");
