...
function scheduleBot(room) {
  ...
  if (p && p.bot && !p.eliminated) {
    ...
    setTimeout(() => {
      ...
      if (room.round > 1 && (p.hand.length <= 3 || handScore(p.hand) <= 8)) return declarePlayer(room, p, true);
...
function roomView(room, username) {
  ...
  return {
    ...
    canDeclare: room.round > 1 && room.status === "playing" && room.turn === room.players.findIndex(p => p.username === username),
    ...
    me: me ? { hand: me.hand.map(publicCard), score: me.score } : null,
...
function declarePlayer(room, p, automatic = false) {
  if (room.status !== "playing" || room.round <= 1 || room.players[room.turn] !== p || p.eliminated) return false;
  clearTurnTimer(room);
  p.declared = true;
