...
function latestMoveLabel(p) {
  ...
  if (!m) return "—";
  if (m.from === "deck") return "Deck";
  if (m.from === "open") return m.card ? `Open • ${m.card.rank}${m.card.suit}` : "Open";
  return "—";
}
function renderHistoryTable() {
  const players = state.players || [];
  const html = players.map(p => `<tr class="${p.username === state.turn ? "latest-turn" : ""}"><td><b>${p.username}</b>${p.bot ? " 🤖" : ""}${p.username === state.me?.username ? " <small>(you)</small>" : ""}</td><td>${latestMoveLabel(p)}${p.lastAction?.automatic ? " <small>auto</small>" : ""}</td><td><b>${p.score}</b></td></tr>`).join("");
  $("historyTableBody").innerHTML = html || `<tr><td colspan="3">No players</td></tr>`;
  const mobile = $("mobileHistory");
  if (mobile) mobile.innerHTML = players.map(p => `<div class="mobile-move ${p.username === state.turn ? "latest-turn" : ""}"><b>${p.username}</b><span>${latestMoveLabel(p)}</span><strong>${p.score}</strong></div>`).join("") || `<div class="mobile-move muted">No players</div>`;
}
...
$("move").onclick = () => {
  ...
  if (!validLeavePreview(cards)) return toast("Invalid group. Use 1 card, 2/4 same-rank, or a same-suit sequence such as 2-3-4, 8-9-10 or 10-J-Q.");
