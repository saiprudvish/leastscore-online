...
function updateMoveUI() {
  ...
  if (declare) {
    ...
    declare.textContent = state?.canDeclare ? "⚑ Declare" : "⚑ Declare (complete opening round first)";
  }
...
function lobbyRender() {
  ...
  $("lobbyPlayers").innerHTML = state.players.map(p => `<div class="lobby-player"><span>${p.bot ? "🤖" : "●"} ${p.username}${p.username === state.me?.username ? " (you)" : ""}</span><small>${p.bot ? "BOT" : (p.connected ? "ONLINE" : "OFFLINE")}</small></div>`).join("");
...
function moveLabel(m) {
  if (!m) return "—";
  const picked = m.picked ? `${m.picked.rank || ""}${m.picked.suit || ""}` : "";
  const left = (m.cards || []).map(c => `${c.rank}${c.suit}`).join(" ");
  return `${m.from === "deck" ? "Deck" : "Open"} → ${picked}${left ? ` • left ${left}` : ""}${m.automatic ? " • auto" : ""}`;
}
function renderHistoryTable() {
  const rows = (state.moveHistory || []).slice().reverse().slice(0, 12);
  $("historyTableBody").innerHTML = rows.length ? rows.map(m => `<tr><td>${m.username}${state.players.find(p=>p.username===m.username)?.bot ? " 🤖" : ""}</td><td>${moveLabel(m)}</td><td>${state.players.find(p=>p.username===m.username)?.score ?? 0}</td></tr>`).join("") : `<tr><td colspan="3">No moves yet</td></tr>`;
  const mobile = $("mobileHistory");
  if (mobile) mobile.innerHTML = rows.length ? rows.slice(0, 5).map(m => `<div class="mobile-move"><b>${m.username}</b><span>${moveLabel(m)}</span></div>`).join("") : `<div class="mobile-move muted">No moves yet</div>`;
}
...
function renderResult() {
  ...
  $("resultMessage").textContent = d.winner ? `${d.username || d.declarer || "The declarer"} declared successfully.` : `${d.username || d.declarer || "The declarer"} declared and lost the declaration.`;
  const deal = $("resultDeal");
  deal.classList.toggle("hidden", !state.canDeal);
...
function render() {
  ...
  if (info) {
    const statusText = state.round === 1
      ? (state.roundReady ? "All players completed the opening round. Declare is now available on the active player's turn." : "Opening round: every player must complete one move before Declare becomes available.")
      : "Declare is available on the active player's turn.";
