...
function connect() {
  ...
  socket.on("state", s => {
    ...
    if (state.status !== "playing" || state.turn !== state.me?.username) { selectedIds.clear(); selectedSource = null; }
...
function validLeavePreview(cards) {
  ...
  // Same-rank group: 2, 3 or 4 cards.
  ...
  if (cards.length >= 3 && cards.length % 2 === 1) {
    ...
    if (vals.includes(1) && vals.includes(13)) return vals.length === 5 && vals.join(',') === '1,10,11,12,13';
...
function updateTimers() {
  ...
  if (state.status === "playing" && state.turnDeadline) {
    ...
    turnTimer.setAttribute("aria-label", `Turn timer ${left} seconds`);
...
function updateMoveUI() {
  ...
  if (move) {
    ...
    move.textContent = selectedSource ? `Move → ${count} card${count === 1 ? "" : "s"}` : "Pick DECK or DISCARD, then select cards → MOVE";
  }
  if (declare) {
...
$("move").onclick = () => {
  ...
  if (!validLeavePreview(cards)) return toast("Invalid leave. Use 1 card, 2–4 same-rank cards, or an odd-length rank sequence: 234, 456, 8910, 10JQ, A2345 or 10JQKA. Suits may be mixed.");
