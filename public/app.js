...
function connect() {
  ...
  socket.on("leftRoom", () => { state = null; selectedIds.clear(); selectedSource = null; selectedPickId = null; localStorage.removeItem("ls_room"); show("lobby"); renderLobbyDefaults(); toast("You left the table."); });
  socket.on("kicked", () => { state = null; selectedIds.clear(); selectedSource = null; selectedPickId = null; localStorage.removeItem("ls_room"); show("lobby"); renderLobbyDefaults(); toast("You were removed from the table by the host."); });
  socket.on("state", s => {
    const oldTurn = state?.turn;
    const oldDeclaration = state?.declaration?.at;
    state = s;
    if (oldTurn !== state.turn) warnedTurnKey = null;
    if (state.status === "playing" && state.turn === state.me?.username && lastTurnKey !== `${state.code}:${state.round}:${state.turn}`) { lastTurnKey = `${state.code}:${state.round}:${state.turn}`; playTurnSound(); }
    if (state.status === "checking" && state.declaration?.at && state.declaration.at !== oldDeclaration) playDeclareSound();
    if (state.code) localStorage.setItem("ls_room", state.code);
    if (state.status !== "playing" || state.turn !== state.me?.username) { selectedIds.clear(); selectedSource = null; selectedPickId = null; }
...
function shareInvite() { if (!state?.code) return; const url=inviteUrl(state.code), text=`Join my LeastScore game! Room code: ${state.code}`; if (navigator.share) navigator.share({title:"LeastScore game invite",text,url}).catch(()=>{}); else navigator.clipboard?.writeText(`${text}\n${url}`).then(()=>toast("Invite link copied.")); }
function shareWhatsApp() { if (!state?.code) return; const text=encodeURIComponent(`Join my LeastScore game! Room code: ${state.code}\n${inviteUrl(state.code)}`); window.open(`https://wa.me/?text=${text}`,"_blank"); }
$("shareLobby")?.addEventListener("click",shareInvite);
$("whatsappLobby")?.addEventListener("click",shareWhatsApp);
$("copyInviteLink")?.addEventListener("click",()=>{ if(state?.code) navigator.clipboard?.writeText(inviteUrl(state.code)).then(()=>toast("Invite link copied.")); });
...
function updateSelectionHint() {
  ...
  if (!selectedSource) el.textContent = selectedIds.size ? `Leaving: ${labels} • choose Deck or a discard card` : "Tap cards to leave • then choose Deck or a discard card";
  else if (selectedSource === "deck") el.textContent = selectedIds.size ? `Leaving: ${labels} • picking from Deck` : "Pick from Deck • then choose cards to leave";
  else el.textContent = selectedPickId ? (selectedIds.size ? `Leaving: ${labels} • picking ${cardLabel((state.openCards || []).find(c=>c.id===selectedPickId))}` : `Picking ${cardLabel((state.openCards || []).find(c=>c.id===selectedPickId))}`) : "Choose a discard card to pick";
}
function updateMoveUI() {
  ...
  ["shareLobby","whatsappLobby","copyInviteLink","copyLobbyCode"].forEach(id => $(id)?.classList.toggle("hidden", !shareAllowed));
