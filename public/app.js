...
function updateMoveUI() {
  ...
  if (declare) {
    ...
    declare.textContent = state?.canDeclare ? "⚑ Declare" : (state?.round === 1 ? "⚑ Declare (after all players move)" : "⚑ Declare");
  }
...
function lobbyRender() {
  ...
  start.textContent = enoughPlayers ? "Start game" : "Waiting for 2 players";
  if ($("lobbyStatus")) {
...
function render() {
  ...
  if (info) {
    const statusText = state.round === 1
      ? (state.roundReady ? "All players completed round 1. Declare is now available on the active player's turn." : "Round 1: every player must complete one move before Declare becomes available.")
