...
async function enter(){try{let j=await api("/api/"+mode,{username:$("username").value,password:$("password").value});token=j.token;localStorage.setItem("ls_token",token);$("welcome").textContent="Hi, "+j.username;show("lobby")}catch(e){$("authMsg").textContent=e.message}}
...
function connect(){socket=io({auth:{token}});socket.on("connect_error",()=>{localStorage.removeItem("ls_token");show("auth")});socket.on("errorMsg",toast);socket.on("state",s=>{state=s;render()})}
$("create").onclick=()=>socket.emit("createRoom");
$("playBots").onclick=()=>{socket.emit("createRoom");setTimeout(()=>{if(state?.code)socket.emit("playBots",{code:state.code,count:Number($("botCount").value)})},400)};
