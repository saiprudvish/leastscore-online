...
function connect() {
  ...
  socket.on("connect", () => {
    ...
    if (saved) socket.emit("rejoinRoom", { code: saved });
  });
  socket.on("connect_error", err => {
    console.error("LeastScore connection error", err);
    toast(err?.message || "Unable to connect to game server.");
  });
  socket.on("errorMsg", msg => toast(msg || "Something went wrong."));
  socket.on("kicked", () => {
    localStorage.removeItem("ls_room");
    state = null; selectedIds.clear(); selectedSource = null; selectedPickId = null;
    show("lobby"); renderLobbyDefaults();
  ...
  socket.on("state", next => {
    ...
    if (pendingBots && !pendingBots.sent && state.code && state.status === "lobby") {
    ...
    if (currentDeclaration && currentDeclaration !== previousDeclaration) playDeclareSound();
    render();
  });
  return socket;
}
